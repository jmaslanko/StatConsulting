...
def recalibrate(raw_data: pd.DataFrame) -> pd.DataFrame:
    '''
    ...
    for column in cols:
        data['original' + column] = data[column]
...
def raw(raw_data: pd.DataFrame) -> pd.DataFrame:
...
def transformed(df: pd.DataFrame) -> pd.DataFrame:
...
def standardized(df: pd.DataFrame) -> pd.DataFrame:
...
def norm_based(df: pd.DataFrame) -> pd.DataFrame:
...
def agg_pcs(df: pd.DataFrame) -> pd.DataFrame:
...
def agg_mcs(df: pd.DataFrame) -> pd.DataFrame:
...
def pcs_mcs(df: pd.DataFrame) -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    data1 = recalibrate(raw_data)
    data2 = raw(data1)
    data3 = transformed(data2)
    data4 = standardized(data3)
    data5 = norm_based(data4)
    data6 = agg_pcs(data5)
    data7 = agg_mcs(data6)
    final_data = pcs_mcs(data7)